class ChessboardController {
  ...
  #toggleCurrentPlayer() {
    ...
    if (this.chessboardModel.currentPlayerColor == PieceColor.WHITE) this.chessboardModel.currentPlayerColor = PieceColor.BLACK
  ...
  #toggleHints(field, enableHints) {
    ...
    moves.get(MoveType.SILENT)?.forEach((move) => {
      this.chessboardView.setFieldType(move.toField.row, move.toField.column, enableHints ? FieldType.SILENT_MOVE : FieldType.NONE)
    })
    moves.get(MoveType.ATTACK)?.forEach((move) => {
      this.chessboardView.setFieldType(move.toField.row, move.toField.column, enableHints ? FieldType.ATTACK_MOVE : FieldType.NONE)
    })
  }
